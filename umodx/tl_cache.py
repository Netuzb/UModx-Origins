...
class CustomTelegramClient(TelegramClient):
    def __init__(self, *args, **kwargs):
        ...
        self._umodx_entity_cache = {}
        self._umodx_perms_cache = {}
        self._umodx_fullchannel_cache = {}
        self._umodx_fulluser_cache = {}
    ...
    async def get_entity(
        ...
        _umodx_client_id_logging_tag = copy.copy(self.tg_id)  # skipcq
        ...
        if (
            not force
            and hashable_entity
            and hashable_entity in self._umodx_entity_cache
            and (
                not exp
                or self._umodx_entity_cache[hashable_entity].ts + exp > time.time()
            )
        ):
            logger.debug(
                "Using cached entity"
                f" {entity} ({type(self._umodx_entity_cache[hashable_entity].entity).__name__})"
            )
            return copy.deepcopy(self._umodx_entity_cache[hashable_entity].entity)

        resolved_entity = await TelegramClient.get_entity(self, entity)

        if resolved_entity:
            cache_record = CacheRecord(hashable_entity, resolved_entity, exp)
            self._umodx_entity_cache[hashable_entity] = cache_record
            ...
            if getattr(resolved_entity, "id", None):
                ...
                self._umodx_entity_cache[resolved_entity.id] = cache_record
            ...
            if getattr(resolved_entity, "username", None):
                ...
                self._umodx_entity_cache[f"@{resolved_entity.username}"] = cache_record
                self._umodx_entity_cache[resolved_entity.username] = cache_record
    ...
    async def get_perms_cached(
        ...
        if (
            ...
            and hashable_entity
            and hashable_user
            and hashable_user in self._umodx_perms_cache.get(hashable_entity, {})
            and (
                not exp
                or self._umodx_perms_cache[hashable_entity][hashable_user].ts + exp
                > time.time()
            )
        ):
            logger.debug(f"Using cached perms {hashable_entity} ({hashable_user})")
            return copy.deepcopy(
                self._umodx_perms_cache[hashable_entity][hashable_user].perms
            )
        ...
        if resolved_perms:
            ...
            self._umodx_perms_cache.setdefault(hashable_entity, {})[
                hashable_user
            ] = cache_record
            ...
            def save_user(key: Union[str, int]):
                ...
                if getattr(user, "id", None):
                    self._umodx_perms_cache.setdefault(key, {})[user.id] = cache_record

                if getattr(user, "username", None):
                    self._umodx_perms_cache.setdefault(key, {})[
                        f"@{user.username}"
                    ] = cache_record
                    self._umodx_perms_cache.setdefault(key, {})[
                        user.username
    ...
    async def get_fullchannel(
        ...
        if (
            not force
            and self._umodx_fullchannel_cache.get(hashable_entity)
            and not self._umodx_fullchannel_cache[hashable_entity].expired()
            and self._umodx_fullchannel_cache[hashable_entity].ts + exp > time.time()
        ):
            return self._umodx_fullchannel_cache[hashable_entity].full_channel

        result = await self(GetFullChannelRequest(channel=entity))
        self._umodx_fullchannel_cache[hashable_entity] = CacheRecordFullChannel(
            hashable_entity,
            result,
            exp,
        )
    ...
    async def get_fulluser(
        ...
        if (
            not force
            and self._umodx_fulluser_cache.get(hashable_entity)
            and not self._umodx_fulluser_cache[hashable_entity].expired()
            and self._umodx_fulluser_cache[hashable_entity].ts + exp > time.time()
        ):
            return self._umodx_fulluser_cache[hashable_entity].full_user

        result = await self(GetFullUserRequest(entity))
        self._umodx_fulluser_cache[hashable_entity] = CacheRecordFullUser(
            hashable_entity,
            result,
            exp,
        )
        ...
        for item in request:
            if item.CONSTRUCTOR_ID in self.__forbidden_constructors and next(
                (
                    ...
                    # APIRatelimiterMod is a core proxy, so it wraps around every module in umodx, if installed
                    # ForbidJoinMod is also a Core proxy, so it wraps around every module in umodx, if installed
                    # LoaderMod prompts user to join developers' channels
                    # umodxSettings prompts user to join channels, required by modules
                ),
                None,
            ):
                logger.debug(
    ...
    async def cleaner(self):
        while True:
            for record, record_data in self._umodx_entity_cache.copy().items():
                if record_data.expired():
                    del self._umodx_entity_cache[record]
                    logger.debug(f"Cleaned outdated cache {record=}")

            for chat, chat_data in self._umodx_perms_cache.copy().items():
                for user, user_data in chat_data.copy().items():
                    if user_data.expired():
                        del self._umodx_perms_cache[chat][user]
                        logger.debug(f"Cleaned outdated perms cache {chat=} {user=}")

            for channel_id, record in self._umodx_fullchannel_cache.copy().items():
                if record.expired():
                    del self._umodx_fullchannel_cache[channel_id]
                    logger.debug(f"Cleaned outdated fullchannel cache {channel_id=}")

            for user_id, record in self._umodx_fulluser_cache.copy().items():
                if record.expired():
                    del self._umodx_fulluser_cache[user_id]
