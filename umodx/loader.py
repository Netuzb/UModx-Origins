...
class InfiniteLoop:
    ...
    def stop(self, *args, **kwargs):
        with contextlib.suppress(AttributeError):
            _umodx_client_id_logging_tag = copy.copy(
...
def tag(*tags, **kwarg_tags):
    ...
    @loader.tag("only_messages", "only_pm", regex=r"^[.] ?umodx$", from_id=659800858)
...
class Modules:
    ...
    async def _junk_collector(self):
        ...
        while True:
            ...
            for module in self.modules:
                commands.update(module.umodx_commands)
                inline_handlers.update(module.umodx_inline_handlers)
                callback_handlers.update(module.umodx_callback_handlers)
                watchers.extend(module.umodx_watchers.values())
    ...
    async def _register_modules(self, modules: list, origin: str = "<core>"):
        with contextlib.suppress(AttributeError):
            _umodx_client_id_logging_tag = copy.copy(self.client.tg_id)
    ...
    def register_commands(self, instance: Module):
        ...
        if getattr(instance, "__origin__", "") == "<core>":
            self._core_commands += list(
                map(lambda x: x.lower(), list(instance.umodx_commands))
            )
        ...
        for _command, cmd in instance.umodx_commands.items():
        ...
        for alias, cmd in self.aliases.copy().items():
            if cmd in instance.umodx_commands:
                self.add_alias(alias, cmd)

        for name, func in instance.umodx_inline_handlers.copy().items():
        ...
        for name, func in instance.umodx_callback_handlers.copy().items():
    ...
    def register_watcher(self, instance: Module):
        ...
        for _watcher in instance.umodx_watchers.values():
    ...
    async def _decline(
        ...
        self._db.set(
            "umodx.main",
            "declined_joins",
            list(set(self._db.get("umodx.main", "declined_joins", []) + [channel.id])),
        )
        event.status = False
        event.set()
        await call.edit(
    ...
    async def _request_join(
        ...
        if channel.id in self._db.get("umodx.main", "declined_joins", []):
            if assure_joined:
                raise LoadError(
        ...
        _module.strings._base_strings["_umodx_internal_request_join"] = (
        ...
        _module.strings_ru["_umodx_internal_request_join"] = (
        ...
        await self.inline.bot.send_animation(
            ...
            caption=_module.strings("_umodx_internal_request_join"),
        ...
        _module.umodx_wait_channel_approve = (
        ...
        with contextlib.suppress(AttributeError):
            delattr(_module, "umodx_wait_channel_approve")
    ...
    async def complete_registration(self, instance: Module):
        ...
        instance.get_prefix = partial(self._db.get, "umodx.main", "command_prefix", ".")
    ...
    async def _mod_import_lib(
        ...
        *,
        ...
        if re.search(r"# ?scope: ?umodx_min", code):
            ver = tuple(
                map(
                    int,
                    re.search(r"# ?scope: ?umodx_min ((\d+\.){2}\d+)", code)[1].split(
                        "."
                    ),
                )
            )
            ...
            if version.__version__ < ver:
                _raise(
                    RuntimeError(
                        f"Library requires umodx version {'{}.{}.{}'.format(*ver)}+"
                    )
                )

        module = f"umodx.libraries.{url.replace('%', '%%').replace('.', '%d')}"
        ...
        if (
            ...
            and self._db.get("umodx.main", "stats", True)
            and url is not None
        ...
        lib_obj.get_prefix = partial(self._db.get, "umodx.main", "command_prefix", ".")
