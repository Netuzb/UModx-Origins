...
class umodx:
        ...
        if "DYNO" not in os.environ:
            self.sessions += [
                ...
                for session in filter(
                    lambda f: f.startswith("umodx-") and f.endswith(".session"),
                    os.listdir(self.arguments.data_root or BASE_DIR),
                )
            ]

        for key, value in os.environ.items():
            if key.startswith("umodx_session"):
    ...
    async def save_client_session(
        ...
        if "DYNO" not in os.environ:
            ...
            key = (
                f"umodx_session_{utils.rand(6)}"
                if "umodx_session" in heroku_config
                else "umodx_session"
            )
        ...
        client.umodx_db = database.Database(client)
        await client.umodx_db.init()
