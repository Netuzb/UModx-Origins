...
class Module:
    ...
    def __getattr__(self, name: str):
        if name in {"umodx_commands", "commands"}:
            return get_commands(self)

        if name in {"umodx_inline_handlers", "inline_handlers"}:
            return get_inline_handlers(self)

        if name in {"umodx_callback_handlers", "callback_handlers"}:
            return get_callback_handlers(self)

        if name in {"umodx_watchers", "watchers"}:
