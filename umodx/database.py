...
class Database(dict):
    ...
    def _postgre_save_sync(self):
        with self._postgre, self._postgre.cursor() as cur:
            cur.execute(
                "UPDATE umodx SET data = %s WHERE id = %s;",
    ...
    async def postgre_init(self) -> bool:
        ...
        with conn, conn.cursor() as cur:
            cur.execute("CREATE TABLE IF NOT EXISTS umodx (id bigint, data text);")

            with contextlib.suppress(Exception):
                cur.execute(
                    "SELECT EXISTS(SELECT 1 FROM umodx WHERE id=%s);",
                    (self._client.tg_id,),
                )

                if not cur.fetchone()[0]:
                    cur.execute(
                        "INSERT INTO umodx (id, data) VALUES (%s, %s);",
            ...
            with contextlib.suppress(Exception):
                cur.execute(
                    ...
                    "WHERE table_name = 'umodx' AND column_name = 'id';"
                )
                ...
                if "integer" in cur.fetchone()[0].lower():
                    ...
                    cur.execute("ALTER TABLE umodx ALTER COLUMN id TYPE bigint;")
    ...
    async def init(self):
        ...
        try:
            self._assets, _ = await utils.asset_channel(
                self._client,
                "umodx-assets",
                "🌆 Your UModx assets will be stored here",
                archive=True,
    ...
    def read(self):
        ...
        if self._postgre:
            try:
                with self._postgre, self._postgre.cursor() as cur:
                    cur.execute(
                        "SELECT data FROM umodx WHERE id=%s;",
                        (self._client.tg_id,),
                    )
                    self.update(
                        **json.loads(
