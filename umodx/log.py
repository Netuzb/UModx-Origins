...
class TelegramLogsHandler(logging.Handler):
    ...
    def dumps(self, lvl: int = 0, client_id: Optional[int] = None) -> list:
        ...
        return [
            ...
            and (not record.umodx_caller or client_id == record.umodx_caller)
        ]
    ...
    def emit(self, record: logging.LogRecord):
        try:
            caller = next(
                (
                    frame_info.frame.f_locals["_umodx_client_id_logging_tag"]
                    for frame_info in inspect.stack()
                    if isinstance(
                        getattr(getattr(frame_info, "frame", None), "f_locals", {}).get(
                            "_umodx_client_id_logging_tag"
                        ),
                        int,
                    )
                ),
