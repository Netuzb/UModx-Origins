...
class CommandDispatcher:
    def __init__(
        ...
        self._me = self._client.umodx_me.id
        self._cached_username = (
            self._client.umodx_me.username.lower()
            if self._client.umodx_me.username
            else str(self._client.umodx_me.id)
        )
    ...
    async def _handle_command(
        ...
        if (
            ...
            and message.chat.title.startswith("umodx-")
            and message.chat.title != "umodx-logs"
        ):
            if not watcher:
    ...
    async def future_dispatcher(
        ...
        *args,
        ...
        _umodx_client_id_logging_tag = copy.copy(self.client.tg_id)  # skipcq
        try:
            await func(message)
