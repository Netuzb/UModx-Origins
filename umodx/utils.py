...
async def answer(
    ...
    **kwargs,
    ...
    if isinstance(response, str) and not kwargs.pop("asfile", False):
        ...
        if len(text) >= 4096 and not hasattr(message, "umodx_grepped"):
            try:
...
async def asset_channel(
    ...
    *,
    ...
    if _folder:
        ...
        try:
            folder = next(folder for folder in folders if folder.title == "umodx")
        except Exception:
            folder = None
...
def get_commit_url() -> str:
    """Get current umodx git commit url"""
    try:
        repo = git.Repo()
